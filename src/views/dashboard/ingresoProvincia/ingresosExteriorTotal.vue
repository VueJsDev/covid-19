                        ...
                        <span class="m-section__sub">
                            <center>
                                <div class="alturaFija">
                                    <h6>Total de ingresos del exterior del Pais</h6>
                                </div>
                            </center>
                        </span>
                        <div class="m-section__content">
                            <center>
                                <div class="circleGreen">
                                    <h2>
                                        {{ computedTotalIncomeAboard }}
                                    </h2>
                                </div>
                                <br>
                                <button 
                                    @click="updateScreen(1)"
                                    type="button" 
                                    class="btn m-btn--pill btn-outline-success">
                                    Ver detalles
                                </button>
                            </center>
                        </div>
    ...
    export default {
        ...
        methods: {
            ...
            updateScreen(code){
                ...
                if(this.GET_TOTALS.Exterior === 0) return;
                ...
                /*
                barChart.scrollIntoView({block: "start", behavior: "smooth", alignTo: true});
                window.scrollTo(0, document.getElementById('barChart').offsetTop);
        ...
        computed: {
            ...mapGetters('ingreosALaProvincia', ['GET_TOTALS', 'GET_DATE_RANGE']),
            computedTotalIncomeAboard(){
                return isNaN(Intl.NumberFormat("de-DE").format(this.GET_TOTALS.Exterior)) 
                    ? '-' 
                    : Intl.NumberFormat("de-DE").format(this.GET_TOTALS.Exterior);
            },

            isDisabled() {
                if(this.GET_TOTALS.Exterior === 0 || this.GET_TOTALS.Exterior == "-") {
                    return true;
                }
                else {
                    return false;
